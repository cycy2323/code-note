...
type Props = {};
export default class RechargeDetailOtherPayView extends Component<Props> {
    constructor(pros) {
        super(pros);
        this.state = {
            inputString: "",
    ...
    _qrCodeView = () => {
        ...
        if (payWayDetailData.qrCodeUrl&&payWayDetailData.qrCodeUrl.length>0){
            return <View style={styles.qrCodeViewStyle}>
                <FastImage source={{
                    uri: ImageUrlManager.dealURI(payWayDetailData.qrCodeUrl),
                    headers: {Host: GBServiceParam.currentHost},
                    priority: FastImage.priority.normal,
    ...
    _inputView = () => {
        ...
        array.push(<View style={styles.inputViewStyle} key={"depositePerson"}>
            <Text style={{fontSize: 12, color: 'white', marginLeft: 5}}>您的其他方式账号</Text>
            <TextInput
                placeholder={"请输入其他方式账号"}
                placeholderTextColor={'rgb(170,170,170)'}
                style={styles.inputTextStyle}
                value={this.state.inputString}
                onChangeText={this._onChangeText}
                ref={(c)=>this.inputString=c}
                onEndEditing={()=>{this.loadBlur(this.inputString)}}
            />
    ...
    _submitBtnClick = () => {
        if (this.state.inputString.length === 0) {
            this.refs.toast.show("请输入其他方式账号")
        } else {
    ...
    _preferentialSubmitBtnCllick = (visible) => {
        ...
        let paramDic = {
            ...
            "account": payWayDetailData.searchId, "result.payerBankcard": this.state.inputString
        };
