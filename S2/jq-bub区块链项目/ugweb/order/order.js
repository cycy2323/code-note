			...
			xw = XWATable.init({
				...
				findFilter: function() {
                    ...
                    if($('#find_paymentway').val()){
                        findbd.paymentway = $('#find_paymentway').val();
                    }
					if($('#find_status').val()){
					...
					if($('#find_begintime').val() && $('#find_endtime').val()){
						...
						findbd.endtime = $('#find_endtime').val()?$('#find_endtime').val():new Date().toJSON();//格式2019-03-21
					}
