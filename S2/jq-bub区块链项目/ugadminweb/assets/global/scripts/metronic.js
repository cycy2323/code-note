...
var Metronic = function () {
    ...
    var handleUniform = function () {
        ...
        $.each($(".positive-numeric"),function(idx,row){ 

            console.log("numeric:"+parseInt($(row).attr('np'), 10))
            if($(row).attr('np')){
                $(row).numeric({ negative: false,decimalPlaces: parseInt($(row).attr('np'), 10)});    
            }else{
                $(row).numeric({ negative: false,decimalPlaces: 2});
            }
