...
import { paidList } from '@/api/agency/orderManage'
...
export default {
  ...
  data() {
    return {
      ...
      tHeader: ['平台商户号', '商户订单号', '商户ID', '订单来源', '银行', '订单金额', '状态', '手续费', '创建时间'], // 表头名字——写法注意：如审核状态：'审核状态:   1:待审核，2.审核中,3.审核通过，4.审核不通过'
    ...
    getOrderLists() { // 5/7 分线程获取数据
      return new Promise((resolve, reject) => {
        ...
        if (this.total === 0) {
          ...
          parmas.pageNo = 1
          paidList(parmas).then((res) => { resolve(res.list) })
        } else {
