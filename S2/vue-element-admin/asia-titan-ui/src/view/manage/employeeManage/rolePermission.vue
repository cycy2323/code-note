  ...
  <div class="app-container role-list">
    <el-form :inline="true" :model="listQuery" class="demo-form-inline">
      <el-form-item label="平台标识">
        <el-select v-model="listQuery.type" style="width: 120px;">
      ...
      <el-form-item label="角色名称">
        <el-input v-model="listQuery.name" clearable style="width: 220px;"/>
      </el-form-item>
    ...
    <el-dialog :visible.sync="dialogPrims" :close-on-click-modal="false" title="绑定权限" width="600px">
      <el-form label-position="right" label-width="190px">
        <el-tree ref="tree" :data="treeData" :default-checked-keys="expandedKeys" :props="defaultProps" show-checkbox default-expand-all node-key="id"/>
      </el-form>
      <span slot="footer" class="dialog-footer">
...
import { fetchList, fetchAdd, fetchUpdate, fetchUpdateRole, fetchQueryMenus, fetchDelete, htmlContent } from '@/api/manage/employeeManage/rolePermission'
...
export default {
  ...
  data() {
    return {
      add: 'employeeManage:rolePermission:add',
      update: 'employeeManage:rolePermission:update',
      bindAuth: 'employeeManage:rolePermission:bindAuth',
      deleteBtn: 'employeeManage:rolePermission:detele',
      tableKey: 0,
      total: 0,
      listLoading: false,
  ...
  methods: {
    ...
    fetchDelete(row) {
      htmlContent({ roleId: row.id }).then(data => {
        var accountContent = data.map(item => item.nickName)
        this.$confirm(`<p>当前角色绑定了账号：${accountContent.join('、')}</p><p>是否继续删除？</p>`, { dangerouslyUseHTMLString: true }).then(() => {
