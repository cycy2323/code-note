    ...
    <el-table
      ...
      <el-table-column :label="$t('merchantList.operation')" align="center" width="500px">
        <template slot-scope="scope">
          ...
          <el-button v-handle="subUnLockPermis" v-if="scope.row.lockFlag===2" type="danger" size="mini" @click="unLock(scope.row)">{{ $t('merchantList.unlock') }}</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  data() {
    return {
      ...
      subUnLockPermis: 'subAccounManage:subAccounList:unLock'
    }
  },
  created() {
    this.getList()
  ...
  methods: {
    ...
    unLock(row) {
      ...
      subUnLock(this.unLockParams).then(() => {
        this.$message({
          message: '操作成功',
          type: 'success'
        })
    ...
    getOrderLists() {
      return exportExcel(this.total, this.listQuery, getSubList, this)// （总条数，查询条件，接口方法）
    },
    handleClose(done) {
      this.resetForm('dataForm')
      done()
