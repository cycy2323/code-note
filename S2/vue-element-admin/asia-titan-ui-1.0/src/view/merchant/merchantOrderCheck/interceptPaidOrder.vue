  ...
  <div class="app-container">
    ...
    <el-table
      ...
      <el-table-column :label="$t('merchantList.orderSource')" align="center">
        <template slot-scope="scope">
          <span class="">{{ scope.row.sourceType==1?"api":"web" }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column :label="$t('table.cardNumber')" align="center">
        <template slot-scope="scope">
          <span class="">{{ scope.row.bankAccountNo }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column :label="$t('table.OrderTime')" align="center">
        <template slot-scope="scope">
          <span>{{ (scope.row.submitDate || '') | parseTime('{y}-{m}-{d} {h}:{i}:{s}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="原因" prop="resMessage" align="center"/>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form
        ...
        <el-form-item :label="$t('table.requestSource')">
          <el-select
            v-model="temp.sourceType"
            style="width: 110px;margin-right: 20px"
            class="filter-item">
            <el-option
...
import { getOrderList } from '@/api/merchant/merchantOrderCheck/interceptPaidOrder'
...
export default {
  ...
  data() {
    return {
      ...
      titleName: '代付拦截订单查询', // excel文件名
      tHeader: ['商户订单号', '平台商户ID', '订单来源：1=api，2=web', '订单金额', '银行', '银行卡号', '持卡人', '下单时间'], // 表头名字——写法注意：如审核状态：'审核状态:   1:待审核，2.审核中,3.审核通过，4.审核不通过'
      filterVal: ['merOrderNo', 'merId', 'sourceType', 'amount', 'bankName', 'bankAccountNo', 'bankUserName', 'submitDate'], // 表头名字对应接口参数
      timeArray: ['submitDate'], // 要转时间格式的数据 如：["创建时间","修改时间"]
      downExcel: 'merchantOrderCheck:interceptPaidOrder:downExcel'
    }
  },
  created() {
    this.getList()
