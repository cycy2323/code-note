    ...
    <el-dialog :visible="dialogStatus" :title="dialogTitle[dialogType]" :close-on-click-modal="false" width="1000px" @close="dialogStatus = false">
      ...
      <el-form v-if="dialogType === 0" :inline="true" label-position="right" label-width="140px" class="demo-form-inline">
        ...
        <el-form-item label="上游编码:">
          <!-- <el-input v-model="obj.code" disabled style="width: 220px;" /> -->
          <span>{{ obj.code }}</span>
        </el-form-item>
        <el-form-item label="上游状态:">
          <el-tag :type="obj.status === 'SUCCEEDED' ? 'success' : 'danger'">
            {{ obj.status | getStatus }}
          </el-tag>
          <!-- <el-input :value="obj.status | getStatus" disabled style="width: 220px;" /> -->
        </el-form-item>
        <el-form-item label="上游数据:">
          <!-- <span style="width: 360px;">{{ obj.data }}</span> -->
          <el-input v-model="obj.data" disabled type="textarea" style="width: 300px;" />
        </el-form-item>
        <el-form-item label="上游消息:">
          <span>{{ obj.message }}</span>
          <!-- <el-input :value="obj.message" disabled type="textarea" style="width: 220px;" /> -->
        </el-form-item>
      </el-form>
      ...
      <!-- 代付审核退款 -->
      <el-form v-if="dialogType === 6" label-position="right" label-width="120px">
        <el-form-item label="平台订单号:">
          <span>{{ orderNo }}</span>
        </el-form-item>
        <el-form-item label="审核状态:">
...
import { getOrderStatus, getBudang, getExamine, getFrozen, getThaw, getFrozenList, getExamineList } from '@/api/manage/orderManage/payOrder'
...
export default {
  ...
  data() {
    return {
      dialogTitle: ['查看订单状态', '补单申请', '审核订单', '冻结订单', '解冻订单', '退款申请', '审核订单'],
  ...
  methods: {
    ...
    getStatusDate(obj, payType) {
      ...
      if (payType) { // 此处为代付
        ...
        getRepayStatus({ orderId: obj.id }).then(response => {
          for (var key in this.obj) {
            this.obj[key] = response[key]
          }
        })
      } else {
        getOrderStatus({ orderId: obj.id }).then(response => {
          for (var key in this.obj) {
            this.obj[key] = response[key]
          }
        })
