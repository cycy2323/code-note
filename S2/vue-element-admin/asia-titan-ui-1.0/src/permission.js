...
NProgress.configure({ showSpinner: false })// NProgress Configuration  禁用进度条

// permission judge function
function hasPermission(roles, permissionRoles) {
  if (roles.indexOf('per:admin') >= 0) return true // admin permission passed directly
...
router.beforeEach((to, from, next) => {
  ...
  if (Vue.prototype.ManageHostAndMerchantHost.indexOf(document.domain) === -1) {
    ...
    if (document.domain === Vue.prototype.MerchantHost) {
      document.title = 'XX商户管理系统'
    } else if (document.domain === Vue.prototype.ManageHost) {
      document.title = 'XX总管理系统'
    } else if (document.domain === Vue.prototype.AgentHost) {
      document.title = 'XX代理系统'
    }
    // const a = getToken() || 'a'
    if (getToken()) { // determine if there has token      getToken()
