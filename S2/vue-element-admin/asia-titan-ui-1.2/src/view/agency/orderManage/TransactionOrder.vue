<template>
  <div class="app-container">
    <el-form :inline="true" class="filter-container inquire-style" >
      ...
      <el-form-item label="代理总收益:">
        <span class="header-title">{{ statisticsData.agentTotalCommission }}</span>
      </el-form-item>
    ...
    >
      ...
      <el-table-column :label="$t('merchantList.createTime')" align="center" prop="createDate">
        <template slot-scope="scope">
...
import { transactionList, payStatis } from '@/api/agency/orderManage'
...
export default {
  ...
  methods: {
    getList() {
      ...
      payStatis(this.listQuery).then(response => {
    ...
    getOrderLists() {
      return exportExcel(this.total, this.listQuery, transactionList, this)// （总条数，查询条件，接口方法）
    },
