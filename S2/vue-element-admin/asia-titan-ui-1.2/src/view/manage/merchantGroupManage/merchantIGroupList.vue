  ...
  <div class="app-container">
    <el-form :inline="true" :model="listQuery" class="demo-form-inline">
      <el-form-item label="商户ID">
        <el-input v-model.number="listQuery.merId" clearable style="width: 200px;" @keyup.enter.native="refreshList"/>
      </el-form-item>
      <el-form-item label="商户组名称">
        <el-input v-model="listQuery.groupName" clearable style="width: 200px;" @keyup.enter.native="refreshList" />
      </el-form-item>
    ...
    <el-table v-loading="listLoading" :key="tableKey" :data="list" border fit highlight-current-row style="width: 100%;">
      ...
      <el-table-column label="通道名称集合" prop="accountName" align="center">
        <template slot-scope="scope">
          <p v-for="(item, idx) in scope.row.accountName.split('/')" :key="idx" style="margin:0">{{ item }}</p>
        </template>
      </el-table-column>
    ...
    <el-dialog :visible.sync="dialogMerchant" :close-on-click-modal="false" title="修改商户列表" width="600px">
      <el-form label-position="right" label-width="120px">
        ...
        <el-form-item label="选择商户ID">
          ...
          <el-input v-model="merIdsStr" type="textarea" placeholder="请输入ID" rows="2"/>
          <p>每个ID之间以&nbsp;&nbsp;'&nbsp;,&nbsp;'&nbsp;&nbsp;号分割</p>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
  ...
  methods: {
    ...
    showMerchant(row) {
      ...
      fetchQuery({ id: row.id }).then(response => {
        ...
        this.merIdsStr = arr.map(item => item.id).join(',')
        this.merchantForm.id = row.id
        this.merchantForm.groupName = row.groupName
        this.listLoading = false
        this.dialogMerchant = true
    ...
    updateMerchants() {
      ...
      this.merchantForm.merIds = str.filter(item => Number(item))
