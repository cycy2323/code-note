  ...
  <div class="withdraw-list app-container">
    <el-form :inline="true" :model="listQuery" class="filter-container" style="width: 100%;background: #FFFFFF;margin-bottom: 20px;padding: 20px 20px">
      ...
      <el-form-item label="平台订单号后10位">
        <el-input v-model="listQuery.orderNoAftTen" clearable style="width: 160px;" @keyup.enter.native="refreshList" @blur="getRule" />
      </el-form-item>
    ...
    <el-table v-loading="listLoading" ref="multipleTable" :key="tableKey" :data="list" :row-class-name="tableRowClassName" border fit highlight-current-row style="width: 100%;" @selection-change="getSelectionChange">
      ...
      <el-table-column label="操作" fixed="right" align="center" class-name="small-padding" min-width="150" max-width="160">
        <template slot-scope="scope">
          <el-button v-handle="reminder" v-if="scope.row.reminderState === 2" type="primary" size="mini" @click="goReminder(scope.row, 1)">审核催单</el-button>
          <el-button v-handle="reminder" v-if="scope.row.reminderState === 2 || scope.row.reminderState === 1" type="default" size="mini" @click="goReminder(scope.row, 0)">清空催单</el-button>
          ...
          <el-button v-if="scope.row.orderState === 0 && outhHandle(sheet)" type="warning" size="mini" @click="showCustomDialog(scope.row, 1)">补单</el-button>
          <el-button v-if="scope.row.orderState === 4 && outhHandle(examine)" type="primary" size="mini" @click="showCustomDialog(scope.row, 2)">审核</el-button>
          <el-button v-if="scope.row.orderState === 1 && outhHandle(freeze)" type="danger" size="mini" @click="showCustomDialog(scope.row, 3)">冻结</el-button>
          <el-button v-if="scope.row.orderState === 3 && outhHandle(thaw)" type="success" size="mini" @click="showCustomDialog(scope.row, 4)">解冻</el-button>
        </template>
      </el-table-column>
    </el-table>
...
import { fetchList, getDayStatistics, getDaySynchronism, getDayReissue, getAgentList, getNotifyList, goReminder, fetchChannelList } from '@/api/manage/orderManage/payOrder'
...
export default {
  ...
  data() {
    return {
      ...
      reminder: 'orderManage:payOrder:reminder',
  ...
  created() {
    fetchChannelList({ type: 1 }).then(response => {
  ...
  methods: {
    ...
    getRule() {
      var orderNo = this.ToCDB(this.listQuery.orderNoAftTen)
      var value = orderNo.match(/[0-9]/g)
      this.listQuery.orderNoAftTen = value.join('')
    },
    ToCDB(str) {
      var tmp = ''
    ...
    goReminder(row, type) {
      var vm = this
      goReminder({ orderId: row.id, operatorState: type }).then(response => {
        if (type === 0) {
          vm.$message.success('清空成功')
        } else {
          vm.$message.success('催单成功')
        }
        vm.getList()
      })
    },
    tableRowClassName({ row, rowIndex }) {
      if (row.reminderState === 2) {
        return 'danger-row'
      } else if (row.reminderState === 1) {
        return 'warn-row'
      }
      return ''
    }
...
<style>
  ...
  .withdraw-list .danger-row{
    background: rgb(247, 139, 139);
    color: #fff
  }
  .withdraw-list .warn-row{
    background: #ebb563;
    color: #fff
  }
  .withdraw-list .warn-row.hover-row, .withdraw-list .warn-row.current-row, .withdraw-list .danger-row.hover-row, .withdraw-list .danger-row.current-row{
    color: #606266
  }
  .withdraw-list .danger-row .el-tag, .withdraw-list .warn-row.el-tag{
    background: #fff;
  }
</style>
