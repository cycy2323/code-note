  ...
  <div class="withdraw-list app-container">
    <el-form
      ...
      <el-form-item label="代理ID">
        <el-input
          v-model.number="listQuery.agentId"
          placeholder="代理ID"
          clearable
          style="width: 200px;margin-right: 20px"
          class="filter-item"
      ...
      <br>
      <el-form-item label="待退款金额:">
        <span class="header-title">{{ statisticsData.sumOrderAmount }}</span>
      </el-form-item>
      <el-form-item label="待退款条数:">
        <span class="header-title">{{ statisticsData.sumCount }}</span>
      </el-form-item>
      <!--3/7-->
      <!--<download-excel-component-->
      <!--:get-order-list="getOrderLists"-->
    ...
    <div class="chulizhong" style="margin-bottom: 20px;">
      <el-button v-waves v-handle="handle" class="filter-item" type="primary" @click="getDayReissue()">批量置为处理中</el-button>
    </div>
    <el-table
      ...
      <el-table-column label="操作" fixed="right" align="center" class-name="small-padding" min-width="130">
        <template slot-scope="scope">
          ...
          <el-button
            v-if="scope.row.orderState === 3 && outhHandle(refund)"
            type="danger"
            plain
            size="mini"
          ...
          <el-button
            v-if="scope.row.orderState === 5 && outhHandle(examine)"
            type="success"
            size="mini"
            @click="showCustomDialog(scope.row, 6, true)">审核
          </el-button>
          <el-button
            v-if="scope.row.orderState === 3 && outhHandle(handle)"
            type="primary"
            size="mini"
            @click="getDayReissue([scope.row.id])">置为处理中
          </el-button>
        </template>
      </el-table-column>
    </el-table>
...
import { fetchChannelList } from '@/api/manage/orderManage/withdrawList'
import { fetchList, failRefundStatis, getDayReissue } from '@/api/manage/orderManage/withdrawPendingRefund'
...
export default {
  ...
  data() {
    return {
      ...
      // exportBtn: 'orderManage:withdrawPendingRefund:export',//没加这个权限
      handle: 'orderManage:withdrawPendingRefund:handle',
      viewStatus: 'orderManage:withdrawPendingRefund:viewStatus',
      refund: 'orderManage:withdrawPendingRefund:refund',
      examine: 'orderManage:withdrawPendingRefund:examine',
  ...
  methods: {
    async getList() {
      ...
      failRefundStatis(this.listQuery).then(response => {
