<template>
  <div class="app-container">
    <el-form :inline="true" :model="listQuery" class="demo-form-inline">
      ...
      <el-button v-handle="syncMerchantPerm" v-waves class="filter-item" type="primary" @click="synchronousMerchantPerm(2)">同步商户权限</el-button>
    </el-form>
...
import { fetchList, fetchAdd, fetchUpdate, updateDetele, synchronousMerchantPerm } from '@/api/manage/employeeManage/authority'
...
export default {
  ...
  data() {
    return {
      ...
      syncMerchantPerm: 'employeeManage:authority:syncMerchantPerm',
      tableKey: 0,
      listLoading: true,
  ...
  methods: {
    ...
    // },
    // 同步商户权限
    synchronousMerchantPerm(type) {
      this.listLoading = true
      synchronousMerchantPerm({ operatorType: type }).then(response => {
        if (type === 2) {
          var arr = []
          var str = ''
          ...
          if (arr.length) {
            str = '同步权限包含：' + arr.join('、') + '，确定同步吗？'
          } else {
            str = '暂无同步权限'
          }
          this.$confirm(str, '同步权限', {
            customClass: str.length > 500 ? 'custom-quanxian' : ''
          }).then(() => {
            if (arr.length) {
              this.synchronousMerchantPerm(1)
            }
          }).catch(() => {})
        } else {
