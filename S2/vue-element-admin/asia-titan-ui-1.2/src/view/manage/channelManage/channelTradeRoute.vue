  ...
  <div class="app-container">
    <el-form :inline="true" :model="listQuery" class="demo-form-inline">
      <el-form-item label="账号ID">
        <!-- <el-input v-model="listQuery.channelAccountId" clearable placeholder="请输入账号ID" style="width: 240px;" onkeyup="this.value=this.value.replace(/\D/g,'')" @keyup.enter.native="refreshList"/> -->
        <el-select v-model="listQuery.channelAccountId" clearable style="width: 160px;" filterable>
          <el-option v-for="(item, idx) in channelAccountList" :key="idx" :label="item.name" :value="item.id"/>
        </el-select>
      </el-form-item>
      <el-form-item label="通道账户号">
        <el-input v-model="listQuery.channelAccountNo" clearable placeholder="请输入通道账户号" style="width: 160px;" @keyup.enter.native="refreshList"/>
      </el-form-item>
      <el-form-item label="通道账户名称">
        <el-input v-model="listQuery.name" clearable placeholder="请输入通道账户名称" style="width: 160px;" @keyup.enter.native="refreshList"/>
      </el-form-item>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding" min-width="200px">
        <template slot-scope="scope">
          <div v-if="scope.row.status !== 3">
            ...
            <el-button v-handle="userT" size="mini" @click="handleModifyStatus(scope.row,'修改类型')">修改类型</el-button>
          </div>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :visible.sync="dialogPvVisible" :title="dialogTitle" :close-on-click-modal="false" width="700px">
      <el-form v-if="dialogTitle === '修改类型'" ref="formUserType" :model="formUserType" label-position="right" label-width="99px">
        <el-form-item label="使用类型">
          <el-select v-model="formUserType.usageType">
            <el-option :value="0" label="所有"/>
            <el-option :value="1" label="接口"/>
            <el-option :value="2" label="后端"/>
          </el-select>
        </el-form-item>
      </el-form>
      <el-form v-else-if="dialogTitle === '修改交易状态'" ref="formStatus" :model="formStatus" label-position="right" label-width="99px">
...
import { fetchList, fetchUpdate, fetchDetail, fetchUpdateBanks, fetchKey, setWeight, channelList, channelUsageType } from '@/api/manage/channelManage/channelTradeRoute'
...
export default {
  ...
  data() {
    return {
      userT: 'manageChannelManage:channelTradeRoute:userType',
      updateW: 'manageChannelManage:channelTradeRoute:updateW',
      updateS: 'manageChannelManage:channelTradeRoute:updateS',
      setBank: 'manageChannelManage:channelTradeRoute:setBank',
      tableKey: 0,
      list: [],
      total: 0,
      listLoading: true,
  ...
  created() {
    ...
    channelList().then(response => {
      this.channelAccountList = response
    })
    this.getList()
  },
  methods: {
    ...
    createConfirm(title) {
      ...
      if (title === '修改类型') {
        channelUsageType(vm.formUserType).then(response => {
          resultTip('修改使用类型')
        })
    ...
    handleModifyStatus(row, title) {
      ...
      if (title === '修改类型') {
        this.formUserType.id = row.id
        this.formUserType.usageType = row.usageType
