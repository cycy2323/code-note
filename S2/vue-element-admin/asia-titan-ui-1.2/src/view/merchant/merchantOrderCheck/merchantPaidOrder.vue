  ...
  <div class="app-container">
    <div class="filter-container" style="width: 100%;background: #FFFFFF;padding: 20px 20px 0 20px">
      <el-form :inline="true" style="width: 100%;">
        ...
        <el-form-item label="订单金额" class="orderAmount">
          <el-col :span="10">
    ...
    <div style="font-size:12px;">
      <span v-handle="count">提交总条数: <span class="order-statistics">&nbsp;{{ listSituation.orderCount===0 ? 0:listSituation.orderCount ? listSituation.orderCount:'_ _' }}&nbsp;</span>，</span>
      <span v-handle="amount">提交总金额:<span class="order-statistics">&nbsp;{{ listSituation.amount===0 ? 0:listSituation.amount ? listSituation.amount:'_ _' }}&nbsp;</span>元 ，</span>
      ...
      <span v-handle="successAmount">成功总金额:<span class="order-statistics">&nbsp;{{ listSituation.succeededTotalAmount===0 ? 0:listSituation.succeededTotalAmount ? listSituation.succeededTotalAmount:'_ _' }}&nbsp;</span>元，</span>
      <span v-handle="successCount">成功总条数:<span class="order-statistics">&nbsp;{{ listSituation.succeededTotalCount===0 ? 0:listSituation.succeededTotalCount ? listSituation.succeededTotalCount:'_ _' }}&nbsp;</span>条</span>
    </div>
    ...
    <el-table
      ...
      <el-table-column label="操作" align="center">
        <template slot-scope="scope">
          <el-button v-if="scope.row.reminderState === 0 && scope.row.enableReminder === 1" type="danger" size="small" @click="goReminder(scope.row)">催单</el-button>
          <el-button v-if="scope.row.reminderState === 1" type="warning" size="small" disabled>等待补单</el-button>
          <el-button v-if="scope.row.reminderState === 2" type="success" size="small" disabled>催单中</el-button>
        </template>
      </el-table-column>
    </el-table>
...
import { getOrderlist, listSituation, goReminder } from '@/api/merchant/merchantOrderCheck/merchantPaidOrder'
...
export default {
  ...
  data() {
    return {
      ...
      successAmount: 'merchantOrderCheck:merPaidOrder:situation:successAmount',
      successCount: 'merchantOrderCheck:merPaidOrder:situation:successCount'
    }
  },
  created() {
    this.getList()
  ...
  methods: {
    ...
    //   if (situation.length) {
    //     listSituation(this.listQuery).then(response => {
    ...
    goReminder(row) {
      var vm = this
      goReminder({ orderId: row.id }).then(response => {
        vm.getList()
      })
    },
    // handleFreeze(row) {
