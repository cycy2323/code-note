  ...
  <div class="recharge">
    ...
    <div v-if="merchantInfo.extendId" class="r-body">
      ...
      <div class="step-cont step-1-cont">
        <h5>填写您的汇款账号</h5>
        <p class="tip">
          <strong style="font-size: 18px;">注意：</strong>
          <span color="">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;1、请务必在汇款备注中填上您的&nbsp;<strong style="font-size: 18px;">转账备注码：{{ orderNoTen }}</strong></span>
          <span color="">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;2、我们会优先处理填写转账备注码的转账，<strong>未填写的会在24小时之后处理</strong></span>
          <span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;3、<strong style="font-size: 18px;">建行</strong>请将转账备注码填在 <strong style="font-size: 18px;">用途</strong> 一栏</span>
        </p>
        <div class="info">
          <h6>持卡人姓名</h6>
      ...
      <div class="step-cont step-2-cont">
        ...
        <div v-if="secondStep" class="info result">
          <div>
            ...
            <textarea id="info1" v-model="info.bankCardName" :style="{height: getHeight(info.bankCardName)}" readonly name=""/>
            <span class="copy" @click="copyToClipBoard('1')">点此复制</span>
          </div>
          <div>
            <span class="lab">银行名称</span>
            <textarea id="info2" v-model="info.bankName" :style="{height: getHeight(info.bankName)}" name=""/>
            <span class="copy" @click="copyToClipBoard('2')">点此复制</span>
          </div>
          <div>
            <span class="lab">支行信息</span>
            <textarea id="info3" v-model="info.subbranceBankName" :style="{height: getHeight(info.subbranceBankName)}" readonly name=""/>
            <span class="copy" @click="copyToClipBoard('3')">点此复制</span>
          </div>
          <div>
            <span class="lab">专属汇款账号</span>
            <textarea id="info4" v-model="info.bankCardNo" :style="{height: getHeight(info.bankCardNo)}" readonly name=""/>
            <span class="copy" @click="copyToClipBoard('4')">点此复制</span>
          </div>
          <div @click="copyToClipBoard('5')">
            <span class="lab">转账备注码</span>
            <textarea id="info5" v-model="orderNoTen" readonly name=""/>
            <span class="copy">点此复制</span>
            <span class="s-c">（请务必在汇款备注中填上！！！）</span>
          </div>
          <div>
            <span class="lab">应付金额</span>
            <textarea id="info6" v-model="info.amount" readonly name=""/>
            <span class="copy" @click="copyToClipBoard('6')">点此复制</span>
          </div>
...
export default {
  ...
  methods: {
    ...
    copyToClipBoard(txt) {
      var Url2 = document.getElementById('info' + txt)
      var value = Url2.value
      if (value) {
        Url2.select()
    ...
    getHeight(value) {
      ...
      return Math.ceil(value.length / 14) * 20 + 'px'
    }
  }
}
</script>
...
<style scoped>
  ...
  .r-body .step-cont .tip span{
    display: block;
    margin-top: 6px;
  }
   .r-body .step-cont .tip strong{
     color: #EC3746;
   }
  ...
  .r-body .step-cont .result textarea{
    border: 0;
    width: 200px;
    line-height: 20px;
    height: 20px;
