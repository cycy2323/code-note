...
(function($) {
    ...
    */
    function ImagesGrid(cfg) {
        ...
        this.times = cfg.times?cfg.times:"";
        this.isAlign = cfg.align;
        this.maxGridCells = (cfg.cells < 1)? 1: (cfg.cells > cfg.cells)? cfg.cells: cfg.cells;
        ...
        this.renderGridItems = function() {
            ...
            for (var i = 0; i < this.images.length; ++i) {
                ...
                this.renderGridItem(this.images[i], i,this.times[i]);
            }
        ...
        this.renderGridItem = function(image, index,times) {
            ...
            var item = $('<div>', {
                class: 'imgs-grid-image item swiper-slide image-wrap',
    ...
    */
    function ImagesGridModal(cfg) {
        ...
        this.renderInnerContainer = function() {
            ...
            var i=0;
            var img=$(".imgpic");
            $(document).bind('DOMMouseScroll mousewheel',".imgpic" ,function(e) {
                var ev=window.event||e;//处理浏览器兼容性问题
                var scrollUnit=Math.max(-1,Math.min(1,(ev.wheelDelta||-ev.detail)));
                var newWith=Math.max(200,Math.min(1400,parseInt(img.css("width"))+(30*scrollUnit)))+"px";
