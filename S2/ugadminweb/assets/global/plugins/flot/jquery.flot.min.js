...
(function($) {
    ...
    function Plot(placeholder, data_, options_, plugins) {
        var series = [], options = {
            colors : ["#EECF52", "#B2EE52", "#cb4b4b", "#4da74d", "#9440ed"],
            legend : {
                show : true,
                noColumns : 1,
                labelFormatter : null,
        ...
        function getOrCreateAxis(axes, number) {
            if (!axes[number - 1])
                axes[number - 1] = {
                    ...
                    options : $.extend(true, {}, axes == xaxes
                        ? options.xaxis
                        : options.yaxis)
                };
            ...
            for (i = 0; i < series.length; ++i) {
                ...
                if (!format) {
                    ...
                    if (s.bars.show || s.lines.show && s.lines.fill) {
                        var autoscale = !!(s.bars.show && s.bars.zero || s.lines.show
                            && s.lines.zero);
                        format.push({
                            y : true,
                            number : true,
        ...
        function measureTickLabels(axis) {
            var opts = axis.options, ticks = axis.ticks || [], labelWidth = opts.labelWidth
                || 0, labelHeight = opts.labelHeight || 0, maxWidth = labelWidth
        ...
        function adjustLayoutForThingsStickingOut() {
                ...
                for (i = 0; i < series.length; ++i)
                    minMargin = Math
                        .max(
                            minMargin,
                            2
                                * (series[i].points.radius + series[i].points.lineWidth
                                / 2))
            }
            var margins = {
                left : minMargin,
            ...
            $.each(allAxes(), function(_, axis) {
                if (axis.reserveSpace && axis.ticks
                    && axis.ticks.length) {
                    if (axis.direction === "x") {
        ...
        function setupGrid() {
            ...
            for (var a in plotOffset) {
                ...
                plotOffset[a] = typeof margin == "number" ? margin : margin[a]
                    || 0
            }
            ...
            $.each(axes, function(_, axis) {
                ...
                axis.show = axisOpts.show == null
                    ? axis.used
                    : axisOpts.show;
                axis.reserveSpace = axisOpts.reserveSpace == null
                    ? axis.show
        ...
        function setRange(axis) {
            var opts = axis.options, min = +(opts.min != null
                ? opts.min
                : axis.datamin), max = +(opts.max != null
                ? opts.max
        ...
        function setupTickGeneration(axis) {
            ...
            if (opts.alignTicksWithAxis != null) {
                var otherAxis = (axis.direction == "x" ? xaxes : yaxes)[opts.alignTicksWithAxis
                    - 1];
        ...
        function drawGrid() {
            ...
            if (markings) {
                ...
                for (i = 0; i < markings.length; ++i) {
                    ...
                    if (xequal || yequal) {
                        var lineWidth = m.lineWidth
                            || options.grid.markingsLineWidth, subPixel = lineWidth
                            % 2 ? .5 : 0;
                        ctx.beginPath();
        ...
        function drawAxisLabels() {
            $.each(allAxes(), function(_, axis) {
                var box = axis.box, legacyStyles = axis.direction + "Axis "
                    + axis.direction + axis.n + "Axis", layer = "flot-"
                    + axis.direction + "-axis flot-" + axis.direction
                    + axis.n + "-axis " + legacyStyles, font = axis.options.font
                ...
                for (var i = 0; i < axis.ticks.length; ++i) {
                    ...
                    surface.addText(layer, x, y, tick.label, font, null, null,
                        halign, valign)
                }
            })
        }
        ...
        function drawSeriesLines(series) {
            function plotLine(datapoints, xoffset, yoffset, axisx, axisy) {
                ...
                for (var i = ps; i < points.length; i += ps) {
                    var x1 = points[i - ps], y1 = points[i - ps + 1], x2 = points[i], y2 = points[i
                        + 1];
                    if (x1 == null || x2 == null)
                        continue;
        ...
        function drawSeriesPoints(series) {
            ...
            plotPoints(series.datapoints, radius, getFillStyle(series.points,
                series.color), 0, false, series.xaxis,
                series.yaxis, symbol);
            ctx.restore()
        }
        ...
        function insertLegend() {
            ...
            if (options.legend.container != null)
                ...
                var legend = $('<div class="legend">'
                    + table.replace('style="', 'style="position:absolute;'
                    + pos + ";") + "</div>")
                ...
                if (options.legend.backgroundOpacity != 0) {
                    ...
                    $('<div style="position:absolute;width:' + div.width()
                        + "px;height:" + div.height() + "px;" + pos
                        + "background-color:" + c + ';"> </div>')
                        .prependTo(legend).css("opacity",
                ...
                if (s.bars.show && !item) {
                    ...
                    for (j = 0; j < points.length; j += ps) {
                        ...
                        if (series[i].bars.horizontal ? mx <= Math.max(b, x)
                            && mx >= Math.min(b, x) && my >= y + barLeft
                            && my <= y + barRight : mx >= x + barLeft
                            && mx <= x + barRight && my >= Math.min(b, y)
                            && my <= Math.max(b, y))
                            item = [i, j / ps]
                    }
                }
        ...
        function drawPointHighlight(series, point) {
            var x = point[0], y = point[1], axisx = series.xaxis, axisy = series.yaxis, highlightColor = typeof series.highlightColor === "string"
                ? series.highlightColor
                : $.color.parse(series.color).scale("a", .5).toString();
        ...
        function drawBarHighlight(series, point) {
            var highlightColor = typeof series.highlightColor === "string"
                ? series.highlightColor
                : $.color.parse(series.color).scale("a", .5).toString(), fillStyle = highlightColor, barLeft;
            switch (series.bars.align) {
                case "left" :
                    barLeft = 0;
                    break;
