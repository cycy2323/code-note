            ...
            xw=XWATable.init(
                {
                    ...
                    checkAllToggle:true,
                    //----------------基本restful地址---
                    restbase: 'bdsysuser?query='+queryCondion,
                    key_column:'userId',
                    // exportxls:{
                    //     title:"工作人员列表"
                    // },
...
//                  },
                    ...
                    findFilter: function(){
                        ...
                        xw.setRestURL(hzq_rest + 'bdsysuser');
                        return filter.rql();
                    },
...
$(document).on("click","#userRole",function(){
   /* $('#find_userroleId').html("");
    ...
    console.log(userid);
    var user = Restful.getByID(hzq_rest+"bdsysuser",userid);
...
//  });
  ...
  /*  var menurole = Restful.findNQ(hzq_rest+"gassysrole")
    ...
    $.ajax({
        type: 'get',
        url: hzq_rest+'bdsysuserrole/?query={"userId":"'+userid+'"}',
        dataType: 'json',
        contentType: "application/json; charset=utf-8",
        async: false,
        success: function(data) {
...
$(document).on("click","#rule_submit",function(){
    ...
    $.ajax({
        ...
        success: function(data) {
            $.each(data,function(index,item){
                 Restful.delByIDR( hzq_rest + "bdsysuserrole",item.userRoleId)
            })
        },
        error: function(err) {
            // alert("find all err");
    ...
    if(roleId){
        ...
        for(var i=0;i<roleId.length;i++){
            ...
            succ = Restful.postData( hzq_rest + "bdsysuserrole",JSON.stringify(userrole))
        }
        if(succ){
        ...
        var box = bootbox.confirm({
            ...
            callback: function (result) {
                if(result){
                    ...
                    var result = Restful.updateRNQ(hzq_rest+"bdsysuser", batchids.join(','),{status:'3',"modifiedTime":new Date(moment().format("YYYY-MM-DDTHH:mm:ss")+"-00:00"),"modifiedBy":JSON.parse(localStorage.getItem("user_info")).userId})
                    //console.log(result)
