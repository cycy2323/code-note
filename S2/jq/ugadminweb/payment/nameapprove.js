	...
	return {
		...
		reload: function() {
			...
			xw = XWATable.init({
				...
				saveColumn:false,
				//----------------基本restful地址---
				restbase: 'ugotcuser?sort=-createdTime&query={"userType":{"$in":'+JSON.stringify(userTy)+'}}',
...
//						unique: "true",
				...
				// 查询过滤条件
				findFilter: function() {
					var find_valiIdNumber, find_status, find_isTrPwd, find_googleVerify, find_valiGooglesecret,find_ugOtcUserId,find_userName;
					...
					// if($('#userType1 option:selected').val()) {
					// 	find_userType = RQLBuilder.equal("userType", $('#userType1  option:selected').val());
					// }

					if($('#ugOtcUserId1').val()) {
						find_ugOtcUserId = RQLBuilder.equal("ugOtcUserId", $('#ugOtcUserId1').val());
					}
					if($('#userName1').val()) {
						find_userName = RQLBuilder.equal("userName", $('#userName1').val());
					}
					var find_userType = RQLBuilder.equal("userType", "1");
					var filter = RQLBuilder.and([
						find_valiIdNumber, find_status, find_isTrPwd, find_googleVerify, find_valiGooglesecret, find_valiGooglesecret, find_userType,find_ugOtcUserId,find_userName
					]);
					xw.setRestURL(hzq_rest + 'ugotcuser?sort=-createdTime');
					return filter.rql();
				}
...
//		return false;
//	}
	 if(data.rows[index].valiIdNumber == 2) {
