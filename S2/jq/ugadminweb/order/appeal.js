...
var Caozuo = function() {
	return {
		f: function(val, row) {
			...
			result += '</button>';
			result += '</div>';
			if (row.status == 1 || row.status == 4) { // 申诉与仲裁中显示审核按钮
...
$(document).on('click','#edit_btn',function() {
	...
	$('#remarks_select').val(data.rows[index].remarks);
		...
		if(data.rows[index].reason) {
			$("#appealreason_select option[value='" + data.rows[index].reason + "']").attr('selected', 'selected');
		}
		if (data.rows[index].reason == '0_B_1_5' || data.rows[index].reason == '0_B_1_2' || data.rows[index].reason == '0_S_1_2') {
            $("#appealreason_select option[value='" + "0_B_1_5,0_B_1_2,0_S_1_2" + "']").attr('selected', 'selected');
		}
});
...
$(document).on('click','#edit_gp',function() {
	...
	$.each(data.rows[index], function(key, val) {
		...
		$("form[name='edit_form1'] select[name='" + key + "']").val(val);
	});
	/*状态：1-处理中,2-已处理*/
	if(data.rows[index].status) {
