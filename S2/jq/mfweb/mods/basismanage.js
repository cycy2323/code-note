...
var businessBuilder = function(val){
    ...
    return "<select id='businessId' name='businessId' class='form-control select2me'>" +
        "<option value='"+businessId+"' selected>"+businessId+"</option>" ;
};
var roleBuilder = function(val){
    var user_info = localStorage.getItem("user_info");
	//获取后先转为json
    var userInfo = eval('(' + user_info + ')');
    // console.log(userInfo);
	//获取登陆名
	var businessId = userInfo.businessId;
    return "<select id='createdBy' name='createdBy' class='form-control select2me'>" +
        "<option value='"+businessId+"' selected>"+businessId+"</option>" ;
};
