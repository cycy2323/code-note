    ...
    return {
        ...
        init: function(xwajson) {
            ...
            if(!UserInfo.userId()){
                ...
                $.ajax({
                    url: hzq_rest+'mfsysuserconfig/'+userconfigid+'?fields={"configValue":1}',
                    dataType: 'json',
                    async: false
                })
                .done(function(data) {
...
//                                        console.log(val.name+"::ll="+val.checked);
                                ...
                                　　 } 
                                    ...
                                    if(sendjson.loginName){
                                        var login = Restful.findNQ( hzq_rest + 'mfsysuser/?query={"loginName":"'+sendjson.loginName+'"}');
                                    ...
                                    if(sendjson.employeeCode){
                                        var login = Restful.findNQ( hzq_rest + 'mfsysuser/?query={"employeeCode":"'+sendjson.employeeCode+'"}');
                                        if(login && login.length){
                                            bootbox.alert("<center><h4>员工代码不能重复</h4></center>")
                                            return false;
                                        }
                                    }
                        ...
                        success: {
                            ...
                            callback: function() {
                                ...
                                $.each($('#__editform select'), function(index, val) {
                                    ...
                                    if (val.name == xwaOptions["key_column"]||!val.name||val.name.length==0) return;
                                    moditem[val.name] = val.value;
                                    moditem["modifiedTime"] = new Date(moment().format("YYYY-MM-DD HH:mm:ss")+"-00:00");
